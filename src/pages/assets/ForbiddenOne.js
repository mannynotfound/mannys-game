...
const ForbiddenOne = () => {
  ...
  useEffect(() => {
    const checkFractions = async () => {
      const fractionsResp = await fetch(`${fetchUrl}/api/fractions/drop-1`, {
        mode: 'cors',
      }).then((resp) => resp.json());
      setFractions(fractionsResp?.fractions);
    };
  ...
  const renderDropImages = () => {
    ...
    return images.map((image, i) => (
      <div
        className={`w-auto h-0 relative fraction-image ${
      ...
      >
        ...
        {fractionMatches[i] && (
          <a
            className="absolute block inset-0 fraction-detail text-xs p-2 overflow-y-auto"
  ...
  return (
    <section
      className="p-8 border-t-2 border-yellow"
      style={{
        paddingBottom: 100,
      }}
    >
