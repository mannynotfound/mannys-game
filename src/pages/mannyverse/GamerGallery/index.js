...
import { Mesh, ColliderMesh, TriggerMesh } from './Meshes';
...
const GamerGallery = ({ onLoad, ...props }) => {
  ...
  const { bestTweets, loadBestTweets, loadingBestTweets } = props;
  const [showBestTweets, setShowBestTweets] = useState(false);
  ...
  return (
    <group ref={group} {...props} dispose={null}>
      ...
      <group name="screens">
        {bestTweets?.length > 0 && (
          <TweetViewer tweets={bestTweets} visible={showBestTweets} />
        )}
      </group>
      <group name="triggers">
        <TriggerMesh
        ...
        <TriggerMesh
          ...
          position={[-7, 1.75, 8]}
          onCollideBegin={() => {
            setShowBestTweets(true);
            if (!bestTweets.length && !loadingBestTweets) {
              loadBestTweets();
            }
          }}
          onCollideEnd={() => setShowBestTweets(false)}
        />
      </group>
